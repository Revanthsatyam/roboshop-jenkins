...
//      }
//
//      sh 'curl -v -u ${nexususer}:${nexuspass} --upload-file ${component}-${TAG_NAME}.zip http://172.31.44.69:8081/repository/${component}/${component}-${TAG_NAME}.zip'
//    }
//  }
//}
...
def release() {
  stage('Release') {
    sh '''
      aws ecr get-login-password --region us-east-1 | docker login --username AWS --password-stdin 369896808580.dkr.ecr.us-east-1.amazonaws.com
      docker build -t 369896808580.dkr.ecr.us-east-1.amazonaws.com/${component}:${TAG_NAME} .
      docker push 369896808580.dkr.ecr.us-east-1.amazonaws.com/${component}:${TAG_NAME}
'''
  }
}